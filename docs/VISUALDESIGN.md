...
The language, once, because the tokens below encode it: **a ruggedised field
terminal that has been kept** — survivalist bunker crossed with hyper-cute anime
cyberpunk, covered in stickers, and *lived in* rather than merely survived.

That last word is the 2026-08-02 refinement, and it is a change of weathering, not
of language. The crate fill moved from Kevlar Olive to **Deep Moss**: the same
olive drab, with something growing on it. **Copper Oxide** joined as the warm
mid-tone for hardware and wear — hinges, screws, rubbed edges — which the palette
had no colour for at all. Everything else held. The reasoning, including what was
deliberately *not* taken, is in [§6b](#6b-inspiration--not-normative).
...
moss-faced disabled state §3/§7 require and it was missing outright.
...
<p align="center"><a href="spore-specimen.png"><img src="spore-specimen.png" alt="Annotated specimen sheet: the eight palette swatches with hexes and measured contrast on each base, the one forbidden pairing shown against its correct alternative, the three interactive sizes drawn at real pixel height with the touch floor marked, and the four-step spacing scale drawn to scale" width="900" /></a></p>

<p align="center"><em><strong>The specimen sheet is generated</strong> —
<code>design/generate.py</code> draws it from <code>tokens.json</code>, so it
cannot drift from the values it illustrates, and the "design tokens in sync" CI
job guards it like everything else. A guideline whose picture is drawn by hand is
wrong within a month and nobody notices.</em></p>
...
| `--moss` | `#3a5a2e` | Deep Moss — crate fills, inert chrome, disabled states — olive drab that something has grown on |
...
| `--copper` | `#b5651d` | Copper Oxide — hardware and wear — hinges, screws, rubbed edges. The warm mid-tone the palette lacked |
...
| | on `--void` | on `--asphalt` | on `--moss` |
|---|---|---|---|
| `--amber` | **OK** 10.80 | **OK** 9.31 | lg 4.27 |
| `--phosphor` | **OK** 14.59 | **OK** 12.58 | **OK** 5.78 |
| `--pink` | **OK** 5.58 | **OK** 4.81 | **XX** 2.21 |
| `--cyan` | **OK** 15.78 | **OK** 13.61 | **OK** 6.25 |
| `--copper` | **OK** 4.56 | lg 3.94 | **XX** 1.81 |
| `--prose` `#d6af5c` | **OK** 9.56 | **OK** 8.24 | lg 3.78 |

**Never put pink on the moss.** 2.21:1 is unreadable, and it is the one combination
the palette invites — hot pink stickers on an ammo crate. Put the sticker on a
`--void` or `--asphalt` patch instead, or outline it. The 2026-08-02 refinement
made this *more* forbidden, not less: it was 2.32:1 against Kevlar Olive.

Amber on the moss is large-text only (4.27:1): headings and buttons, never body copy.

Copper is hardware, not text. It reads on `--void` and, for large text only, on `--asphalt` — and it is forbidden on the moss (1.81:1), which is exactly where a hinge would sit. Draw wear with it; do not label with it.
...
| Pause · Resume · Copy · Share · Add | **Secondary** radio-switch, moss/asphalt face, amber label | Same height as primary — that is what makes them read as one system |
...
at the corners in `--moss`. Focus raises a 2px `--cyan` ring; do not remove the
outline, thicken it.

**Cursor.** Blinking `--pink` block, 1.06 s cycle. Where a glyph fits, a pixel skull
💀 or heart ♥. Under `reduce`, solid, no blink.

**Button — the radio switch.** Chunky, `--moss` face, 3px hard drop-shadow,
...
**Progress — segmented LED.** Discrete blocks, `--phosphor` filled, `--moss`
...
**The Solarpunk Refinement, and what was taken from it.** The proposal was to
re-read the weathering as hopeful rather than bleak — moss on the crate, copper
oxide on the brass, hardware repaired and kept rather than merely survived — with
a palette of Deep Moss, Copper Oxide, Aged Amber and a Refined Phosphor Green
replacing the neon.

Its own artwork is the argument for how far to take it: the before/after shows
**the same crate**, weathered differently. Not a new object, not a new language.
So it was adopted as a refinement and not as a replacement, and the four decisions
are recorded here because a palette nobody can explain is one the next person will
quietly undo.

| Proposed | Decision |
|---|---|
| Deep Moss replaces Kevlar Olive | **Taken.** `#4b5320` → `#3a5a2e`. The one colour carrying "wasteland" most literally now carries "kept". Every contrast promise on it was re-checked and re-graded, not assumed |
| Copper Oxide joins | **Taken**, and it was the genuinely missing piece: between asphalt and amber the palette had *nothing*. Copper is the hardware — hinges, screws, rubbed edges |
| Refined Phosphor replaces the neon | **Declined.** `--phosphor` is 14.59:1 on void, the best contrast in the palette, and it is the colour that means *it worked*. Softening it spends legibility on mood |
| Pink and cyan dropped | **Declined.** Pink is the entire kawaii half of the language — without it this is Meshtastic with better type. Cyan is the focus ring, which §0.2 and B7 make non-negotiable |

**Moss made the forbidden pair more forbidden**, which is the sort of thing worth
checking rather than hoping for: pink on the crate fill went from 2.32:1 to
2.21:1, so "never pink on olive" survives the refinement with a wider margin.
Amber on it moved 4.48 → 4.27, still large-text-only. `design/generate.py` verified
all of it in both directions — a pair claimed forbidden that had quietly become
readable would have failed the build just as loudly.
