...
| M11-C Build the relocated fountain, and the push threshold | ⬜ | **Both measurements are in and both point the same way.** Delivery: a link dropping 10% of frames loses ~46% of fragmented messages, and repetition redundancy recovers 70% for 40% extra traffic where an erasure code over the same redundancy gives 97% (`linkfrag-loss-*`). CPU, which the delivery numbers cannot see and which now runs *per hop*: fountain decode costs 1.4–2.4× plain reassembly, and — the number that decides it — **single-digit parts per million of the airtime needed to receive the same pieces** (`examples/decode_cost.rs`). The radio outweighs the arithmetic by five to six orders of magnitude, so CPU is not an argument against relocating the fountain even on an Xtensa core. Owed: build it in `linkfrag`, re-measure delivery against the recorded baseline, and the **push-N threshold**, which needs M11-E to exist before anything can be pushed |
