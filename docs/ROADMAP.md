...
| Migrate the RustCrypto stack (blake2 0.11, chacha20poly1305 0.11, aead 0.6) | ⬜ **blocked upstream** | The `rand` half is **done, and not by migrating** — see below. What remains needs `crypto_box` to move to aead 0.6, and its only such release is `0.10.0-pre.0`. Taking a pre-release for the crate that implements `crypto_box_seal` — every sealed DM and every sealed file — is a worse trade than waiting, and `cargo audit` is clean, so there is no pressure to. `blake2` 0.11 could go alone but would put `digest` 0.11 beside `digest` 0.10 for no benefit until the family moves together. Revisit when `crypto_box` 0.10 is released |
