...
| Property-test the crypto state machines (#278) | 🟡 partial | The ratchet is done and **found a real one**: `decrypt` committed state before authenticating, so one flipped bit in a header made the receiver ratchet to an attacker-chosen key and no genuine message ever opened again. Now covered by property tests over a channel that loses, reorders and duplicates, plus the two forgery cases. Still owed: the same treatment for **mix** (§9) and for lock ordering across `Hub`, which are the other two state machines the audit named |
