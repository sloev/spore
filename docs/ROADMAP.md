...
| ~~Migrate the RustCrypto stack~~ | ✅ | `ed25519-dalek` 3, `x25519-dalek` 3, `blake2` 0.11, `chacha20poly1305` 0.11, `aead` 0.6. Unblocked by removing `rand` first — with no direct `rand`, the `rand_core` 0.6/0.9 split that made the bump uncompilable no longer exists. Three things were needed beyond the version numbers, and the second is the one that would have shipped a broken browser: **(1)** `Blake2bVar` is gone in 0.11, replaced by `Blake2b<OutSize>` where the size is a type — all four sizes here are compile-time constants, so three runtime `unwrap()`s disappeared. **(2)** `chacha20poly1305`'s *default* features enable `crypto-common/getrandom`, which drags in `getrandom` 0.4, which refuses `wasm32-unknown-unknown` without `wasm_js` — i.e. without wasm-bindgen, which Part IV says the blob must not need. We generate our own nonces, so `default-features = false` drops it and the wasm stays bindgen-free. **(3)** `curve25519-dalek` 5 is far slower unoptimised than 4 was: the signature-heavy tests went 18s → 151s. `[profile.dev.package.curve25519-dalek] opt-level = 3` brings the suite to 8.7s, *faster* than before the bump. Measured costs that remain: release verification ~22% slower, and the wasm blob +4.1%, the latter because `crypto_box` 0.9 still pulls `curve25519-dalek` 4 alongside 5 — that resolves when `crypto_box` 0.10 ships |
| ~~`rand` 0.9 migration~~ — **`rand` removed instead** | ✅ | Dependabot's bump (#255) could not compile, and the reason was structural rather than incidental: `rand` 0.9 carries `rand_core` 0.9 while `ed25519-dalek` 2 and `crypto_box` 0.9 carry `rand_core` 0.6, so bumping either side puts two incompatible `rand_core`s in one build. The fix was to notice that **every single use was `OsRng.fill_bytes(&mut buf)`** — a buffer of bytes, never a generator handed to a crypto crate, never a distribution or a shuffle. One site did hand an RNG over (`x25519-dalek`'s `random_from_rng` in `ratchet.rs`) and takes the same 32 bytes via `from`. So `rand` was a wrapper around `getrandom`, which the wasm build was already using directly. `crate::fill_random` is now the crate's whole randomness API, `getrandom` is an ordinary dependency on every target, and the version conflict cannot recur because the versions are no longer in the tree to disagree |
| Property-test the crypto state machines (#278) | ✅ | All three machines the audit named, and **the two crypto ones each turned up a live fault**. Ratchet: `decrypt` committed state before authenticating, so one flipped bit in a header made the receiver ratchet to an attacker-chosen key and nothing genuine opened again. Mix: the batch threshold counted messages *held* rather than *due*, so a lone message could leave by itself, and release preserved arrival order, so an observer could correlate by position without any timing at all. `Hub` is a concurrency property and wanted a different technique: a test-only tracker asserts on every take that no other lock **of that hub** is held, driven by a test that calls all fifteen public methods. It found nothing — the discipline held — but it was a comment before and is now checked. All fixed, and the tests emit `docs/STATE_MACHINES.md`: every edge on that page was observed rather than drawn |
| Kernel flow diagrams, generated from real calls | ✅ | [`docs/KERNEL_FLOWS.md`](KERNEL_FLOWS.md) — four scenarios (a flood, a duplicate, custody, and what the kernel refuses), every arrow recorded from a real `Node` call by `examples/kernel_flows.rs` and diff-checked in CI. The spec says what the rules are and the threat model says what they defend; neither showed a message *moving*. Writing it caught three of my own errors in the same PR, which is the argument for generating rather than drawing it |
| Conformance vectors beyond the envelope (#278) | ✅ | `reference/versioned_vectors.json` now pins link framing (with KISS), the INV/WANT payload shapes, the three manifest encodings, and the forwarding decisions; `reference/spore_t1.py` is a pure-Python decoder that exercises them, checked by `test_t1.py` in CI. A **second** file rather than more keys in the frozen one, because the two are different promises: `vectors.json` changes only on a `ver` bump, this one may change with a minor release. Putting them together would also make every legitimate link-framing change carry `allow-frozen-change`, which is how a guard stops meaning anything. The forwarding table is **observed** — each case drives a real node — and that caught two errors in my own prose draft: it had `hops` counting up, and it claimed a failed signature stops a relay. It does not, and must not (§5: verify before binding trust state, do not verify to forward) |
| Identity compromise: decide, do not drift (#278) | ⬜ | A stolen seed signs as that address forever. The threat model says so plainly and says a real answer is a new wire feature — but that is a description, not a decision, and the difference matters for anyone choosing whether to depend on this. Owed: either scope it out in the spec with the reasoning, or design the successor statement. Group keys already have `rotate`/`rekey_seal`/`contribute`; identity has nothing, and the asymmetry should be deliberate rather than incidental |
| A security maturity matrix (#278) | ✅ | [`docs/SECURITY_MATRIX.md`](SECURITY_MATRIX.md), twelve components × fuzzed / property-tested / independently reviewed / hardware, **generated** by `scripts/security_matrix.py` and checked in CI — a hand-written maturity table fails in the direction that flatters, keeping a tick long after the target that earned it was deleted. Each fuzz target declares its coverage in a `//! fuzz-covers:` line and a name that matches no component fails the script rather than quietly reading as *not fuzzed*. Writing it down immediately paid: **link fragmentation had no adversarial input at all** — `fragment_reassembly` covers the fountain, a different module with a different header — and the **store**, the one table an attacker attacks by filling it, had neither fuzzing nor property tests. Both now closed, so every component has at least one form of coverage. The **independently reviewed** column is empty for all twelve and that is the point of the page |
| Move retired designs out of the normative spec (#278) | ⬜ | §3 still spends most of its length on the end-to-end fountain, which nothing emits and nothing parses. It is useful history and it is in the wrong place: a reimplementer reading Part I should meet only rules that bind them. Owed: a short statement of what `b2 FRAGMENT` means and why it is reserved, with the history moved to an appendix or to `git log` |
...
| Task | Status | Notes |
|---|---|---|
| ESP32 keeps its identity across a power cycle | ✅ | From [#303](https://github.com/sloev/spore/issues/303), which put it second: `main.rs` called `Node::new` every boot, so a board changed address every power cycle and could not be left anywhere — nobody can reply to it, and every neighbour sees a stranger each time the lights flicker. Seed and prekey ring now live in **NVS**, not in the `spore` SPIFFS partition: that partition mounts with `format_if_mount_failed`, which is what makes a never-flashed board work out of the box and what would erase the identity along with a damaged store. The ring is re-written when rotation changes it, compared first rather than written on a timer, because NVS has finite erase cycles. **Not verified on hardware** — there is no board here and the crate only builds under Espressif's Xtensa fork, so CI is the first compiler to see it; HARDWARE.md row 21 is the re-run that would settle it |
...
| ~~G4~~ | ~~No drafts~~ — **done**, `communicator::draft`, scoped per conversation and per feed | ~~composer~~ |
| ~~G5~~ | ~~No wasm storage port~~ — **done**. `src/wasm.rs` supplies `JsSpill` over four host imports (`spore_store_{put,get,remove,ids}`), with the same `MAX_ADOPT_BYTES` bound the filesystem backend applies and the same re-verification of every adopted entry against its content-derived id, so a host that lost or altered bytes contributes fewer rather than injecting one. Nothing about the trait, the store or adoption changed | ~~G1–G4~~ |
...
G5 **was** the true blocker and is done, which unblocks G1–G4: they are now
ordinary work rather than work waiting on a capability. G6 is the remaining
structural one.
...
| Task | Status | Notes |
|---|---|---|
| M10-A Host storage port so the browser can persist | ✅ | Was missing from this table entirely while being named the blocker for everything below it. `src/wasm.rs`'s `JsSpill` is the port: the host owns the store exactly as it owns the clock and the CSPRNG, and the core re-verifies every adopted entry against its content-derived id, so a host is not trusted to have kept what it was handed |
| M10-B `communicator` module in `src/`: Identity, Thread, Topic, Bridge, Transfer, Contact stores | 🟡 partial | **Thread done** (G1 + G3): `src/communicator/thread.rs`, a port of `web/app/stores/threads.mjs` whose tests mirror `threads.test.mjs` case for case, so "unchanged behaviour behind an unchanged interface" is checked rather than claimed. Gated behind a default-on `communicator` feature, off for ESP32, and the four portability rules are now **enforced** — including rule 4, the one with no compile guard, because `SystemTime::now()` compiles for wasm and panics at runtime. **Contact done** (G2): `src/communicator/contact.rs`, the store that holds no SPORE data at all — every field is something the user decided locally, and `label_for` deliberately never falls back to an announced name, because a claim that silently becomes "the contact's name" is the confusion the store exists to prevent. `contact_rows` joins the two and flags which is which. One **documented divergence**: the JS sorts with `localeCompare` and this sorts by codepoint, because matching it needs ICU-grade collation data and rule 3 is no new dependencies — asserted in a test against the German case rather than left as a claim. **Topic done**: `src/communicator/topic.rs`. A topic address is `topic_of(name)`, a hash, so a name cannot be recovered from an address — the kernel owns which topics are followed and this store owns what they are called, and keeping them apart is what makes a drifted local list show up as *a topic with no name* rather than as a UI confidently claiming a subscription the node does not have. `name_matches` is the one self-check available: a name is only right if it hashes to the address it is filed under. Posts are capped at 200 per topic, re-applied **on decode**, because the bound belongs to the running node rather than to a blob it was handed. The JS `receive` fell back to `Date.now()` when no timestamp was given; that could not come along, since `SystemTime::now()` compiles for wasm and panics at runtime, so `at` is required. **Drafts done** (G4): `src/communicator/draft.rs`. The one store with no JS predecessor, because the behaviour did not exist — `main.mjs` cleared `chatDraft` on every navigation, so opening another conversation discarded what you had typed, and a reload discarded all of it. Scoped, so a conversation draft and a feed draft cannot collide when their addresses do (both key spaces are truncated SHA-256). Still owed: Identity, Bridge, Transfer |
| M10-C Collapse `wasm.rs` / `ffi.rs` / `android-jni` onto one app-level command+event ABI | 🟡 partial | **The command half exists** for the M10-B stores: `src/communicator/api.rs` is one `call(cmd_bytes) -> response_bytes` entry point behind **three** wasm exports rather than the forty a method-per-function ABI would need. Bytes in, bytes out, so the same call works unchanged over IPC — which is what M10-F needs, since the screens cannot tell a wasm node from a native daemon when bytes are all that cross. `web/comm-test.mjs` drives it from JS against the real module in CI, because a Rust test never crosses the boundary and the boundary is the whole claim. **The web thread store now delegates to it**: `web/app/stores/threads.mjs` is a shim over address translation and the host's storage, its ~190 lines of duplicated conversation logic deleted rather than ported, and the 13 assertions in `threads.test.mjs` pass unchanged against the Rust — which is what makes "behind an unchanged interface" a checked claim. **All three web stores now delegate to it** — threads, contacts and topics are shims over address translation and the host's storage, with ~490 lines of duplicated logic deleted rather than ported, and all 31 of their assertions passing unchanged against the Rust. Still owed: the **event** half, the kernel commands (only the app stores are behind it so far), and re-pointing `ffi.rs` at it. **Android can now reach it**: `nativeCommCall` is one JNI method taking command bytes and returning response bytes, so the phone and the browser run the same store rather than two that agree by coincidence. `ffi.rs` is a separate question — `android/jni` is a Rust crate that depends on `spore` directly and never touches the C ABI, so the C/Python/Go/JS wrappers need `bindings/generate.py` taught about opaque handles first, and no consumer is asking for that yet |
...
| M10-E Re-point Android Kotlin + CLI at the shared layer; delete duplicated logic | 🟡 partial | **The address book is first**: `Petnames.kt` is now a shim over `communicator::contact` through `Comm.kt`, the Kotlin half of the command ABI, with a one-time migration off SharedPreferences that **keeps the old entries** rather than deleting them — they are a few hundred bytes and the only copy if anything is wrong, including for a user who installs an older build tomorrow. The public surface did not change, so no screen changed. Still owed: chat and feed state, and the CLI |
...
**Three of the six stores may not belong here, checked against the tree.** The
list came from the Phase-2 architecture definition; the code disagrees with it in
three places, and building stores to complete a list is how a layer gets things
nobody asked it to hold.

| | What it would own | Finding |
|---|---|---|
| **Transfer** | in-flight file progress | `SporeClient._transfers` holds **no state** — it is a change-detection cache over `node.files()`, which the kernel already owns. Moving it would move an event debouncer, not a store |
| **Bridge** | link health and counters | The transports are host objects (a `WebSocket`, an Android `BluetoothSocket`) and cannot cross. What is left is four counters incremented **per frame**, so moving them puts a wasm call on the hot path to do arithmetic |
| **Identity** | seed, address, petname | All kernel-owned already. The only genuinely missing thing is *multiple* identities, which is a feature rather than a relocation, and is the one the roadmap already calls blocked |

None of these are refusals — if a second host turns out to duplicate bridge
accounting, that is evidence and this table should change. They are recorded so
...
| Task | Status | Notes |
|---|---|---|
| The daemon is a node that can be left running | ✅ | From [#303](https://github.com/sloev/spore/issues/303), which put it first: "smallest code, largest lie if left". The daemon called `Node::new` every start and never `set_spill_dir`, so **a restart produced a new address and an empty store** — the reference node was the one runtime that could not be left running, while Android and ESP32 both persisted long ago. Seed, prekey ring and spill directory now live in `$SPORE_HOME`. `spore --help` exists and exits 0. `spore <bridge-spec>` runs a node with one bridge, which is what every hardware procedure has always told people to type and what none of them could do — the binary read its first argument as a file path, so `spore broadcast` answered "cannot read config `broadcast`". A test scans the documentation and parses every `spore …` it finds, so the commands cannot rot apart from the code again |
...
| Task | Status | Notes |
|---|---|---|
| A carrier that never offers what it holds | ✅ | **The INV half of §6 had no production caller.** `build_inv` was called only from tests: a node answered an INV it received and never sent one, so a message handed to a courier reached its destination only if the courier happened to be the original sender, whose own unacked traffic `resend_unacked` re-floods. Custody — the behaviour this protocol is named for — did not complete over a live link. Found while looking for somewhere to put the watermark. `tick` now offers what the node is carrying on an `INV_OFFER_SECS` cadence, a node with nothing stays quiet, and the cadence starts on first tick rather than firing the moment a node boots. `build_inv` is also now capped at `MAX_IDS_PER_GOSSIP` like the side that reads it — it had no bound at all, which did not matter while nothing sent one |
| Per-neighbour INV watermark | ⬜ | [SPEC §6](SPEC.md) claimed this existed and nothing implemented it — found by writing the glossary and trying to define the word. `build_inv` filters by topic and custody only, so two nodes that meet repeatedly re-offer each other the same ids every time, and the peer re-declines them. Bounded by `MAX_IDS_PER_GOSSIP`, so it is waste rather than a vulnerability. Owed: remember per neighbour what was already offered and not wanted, with a bound on the remembering — which is the part that needs thought, since a table keyed by neighbour is a table a stranger can grow |
| M14-A Glossary | ✅ | [`docs/GLOSSARY.md`](GLOSSARY.md), 23 terms, each pointing at where the thing lives so a definition can be checked rather than believed. Writing it found two claims the code does not honour — see the watermark row above, and the landing page describing an envelope as carrying an *expiry*, which M12 replaced with `created_at` over a year of commits ago. Both fixed here |
| M14-B Concepts page | ✅ | [`docs/CONCEPTS.md`](CONCEPTS.md) — each metaphor on the landing page, then the thing it is actually made of, then where the rules are. It deliberately spends most of its length on the decisions that read backwards until explained: why a message says when it was born rather than when it should die, why a relay forwards what it cannot verify, and why there is no group membership list |
...
| M14-D Client-side search | ✅ | `site/search.html`, 326 sections indexed. **No Lunr and no CDN**: the docs are a few hundred sections where a linear scan is instant, and a CDN script cannot be used at all since the site must work opened from disk — so the index is inlined (124 KB) rather than fetched. Results are *sections*, not pages, so a hit lands on the paragraph rather than the top of a long document. The ranking is one file shared between the shipped page and its test, and the build additionally drives the emitted page in jsdom, because a ranking test passes perfectly against an index of zero entries |
| M14-E Navigation aids in long normative docs | ✅ | Breadcrumbs on all 23 doc pages — `SPORE / Developer / <group> / <page>` — generated by parsing `docs/DEVELOPER.md`, so a row moved between groups moves the crumb with it and a page with no row gets no crumb rather than a wrong one. Marked up as an `ol`, because the separators are CSS `::before` and a screen reader would otherwise read "SPOREDeveloperThreat model" as one word. In-page contents extended to the threat model, glossary, concepts and dev guide; the threat model is as long as the spec and had none, so finding one adversary meant scrolling past five |
| M14-F Tables readable on a phone | ⬜ | The spec and threat model tables do not wrap or scroll cleanly on a small screen |
| M14-G Rewrite the Developer hub intro | ✅ | "Pick a row" is gone. The hub now says how the groups are ordered — what is this, what are the rules, how do I build it, what is proved, where is it going — and points at Search for a specific word. The 24 rows are regrouped from four sections into five with a purpose line each, and **Concepts, Glossary, Kernel flows and State machines were filed under "Security & verification"**, which is where a reader would never look for them |
...
| M14-I "What works today" summary | ✅ | [`docs/STATUS.md`](STATUS.md), **generated** by `scripts/status_summary.py` from the bridge status rows, the hardware checklist and the security matrix, and checked in CI. A hand-written status page ages in the flattering direction — something is marked working when its tests pass and nothing moves it back. Writing it caught two ways of doing exactly that: three bridges were silently dropped because they used a marker the parser did not know, and Meshtastic was filed as fully tested because its status line *starts* with ✅ while two of its four pipes have never run on hardware. Mixed statuses now get their own bucket and keep their whole line |
