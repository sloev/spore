...
| Property-test the crypto state machines (#278) | ⬜ | The parsers are well fuzzed; the *state machines* are not tested at all for the transitions that would actually hurt. Owed: a ratchet fed reordered, lost and duplicated messages; simultaneous transitions from both ends; restart mid-session; replay of a captured message after a rotation; and what a key compromise does and does not reach. This is the testable half of "ratchet, mix and lock ordering are unreviewed" above, and the more useful half to do first — a property test finds what a reading misses, and leaves a regression guard behind |
