...
| Conformance vectors beyond the envelope (#278) | ⬜ | `reference/vectors.json` pins identity, the envelope, its id and signature, armor, and the file layer's content id. It does **not** pin forwarding decisions, INV/WANT payload shapes, link framing or the manifest encoding — so two implementations could agree perfectly on an envelope and still fail to talk. Owed: worked vectors for each, and a decoder that exercises them, so "wire-compatible" means the whole protocol rather than its first sixteen bytes |
