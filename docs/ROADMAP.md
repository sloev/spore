...
| Property-test the crypto state machines (#278) | 🟡 partial | Ratchet and mix done, and **each turned up a live fault**. Ratchet: `decrypt` committed state before authenticating, so one flipped bit in a header made the receiver ratchet to an attacker-chosen key and nothing genuine opened again. Mix: the batch threshold counted messages *held* rather than *due*, so a lone message could leave by itself, and release preserved arrival order, so an observer could correlate by position without any timing at all. Both fixed, both covered by property tests, and the tests emit `docs/STATE_MACHINES.md` — every edge on that page was observed rather than drawn. Still owed: **lock ordering across `Hub`**, the third machine the audit named, which is a concurrency property and wants a different technique than these two |
