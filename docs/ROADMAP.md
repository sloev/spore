...
| M11-M Chunks are content-addressed (#256.4) | ✅ **prerequisite done; chunking itself still open** | Scoped as "chunks are already content-addressed, so dedup falls out with no new mechanism". They were not: a byte-identical file published twice shared **0 of 16 envelopes** and got a different magnet, because a chunk's name was the hash of its whole envelope and that envelope carried a random per-publish `file_id`, a topic derived from it, and a wall-clock expiry. Fixed by separating the two namespaces the file layer had been conflating — an **envelope id** names a message and must cover expiry and dest; a **content id** names bytes. A chunk payload is now `[CHUNK_TAG][bytes]`, manifests name content ids, and the store carries a `content → envelope` index rebuilt on spill adoption. A sealed part's AEAD nonce is its position in the walk, not a field in the payload. Result: 40 kB of one repeated byte is thirty chunks and **two** stored objects, and two publishers an hour apart can serve each other's fetchers. What remains of the original item is the FastCDC boundaries themselves, which now have something to build on — and which need chunking parameters fixed by the protocol rather than derived from the publisher's MTU, or two publishers still cut differently and share nothing |
