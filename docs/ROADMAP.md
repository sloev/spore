...
| M11-C Loss recovery for link fragments, and the push threshold | ⬜ | **Measured, and the answer is a real erasure code.** An envelope split into n pieces survives only if all n arrive, so a link dropping 10% of frames loses ~46% of fragmented messages and one dropping 20% loses ~64% — `spore-sim`'s `linkfrag-loss-*` scenarios, 900 B over a 237-byte link, 200 attempts each, tracking `(1-p)^n` as theory says they should. Repetition redundancy was measured as the cheapest candidate that needs no return path: at 10% loss and 40% extra traffic it recovers 70%, where an ideal erasure code over the same redundancy gives 97%; repetition needs 140% extra traffic to reach 86%. So relocate the fountain rather than repeat frames — and `src/fountain.rs` already exists, which makes it the cheap thing to try. Still owed: build it at the link layer and re-measure; compare against hop-local NACK on media that have a return path; and the **push-N threshold**, which cannot be measured until M11-E exists to push anything |
