...
| M11-M Content-defined chunking (#256.4) | ⬜ **blocked — premise was false** | Scoped as "chunks are already content-addressed, so dedup falls out with no new mechanism". **They are not.** Measured by `a_file_published_twice_shares_nothing_with_itself`: a byte-identical file published twice shares **0 of 16 envelopes**, and gets a different magnet — even from the same node, one line later. A chunk's id is the hash of its whole envelope, which carries `file_id` (16 `OsRng` bytes minted per publish) in the payload, a per-file topic derived from it in `dest`, and a wall-clock `expiry`. Chunks are *publish*-addressed. Cutting smarter boundaries on top of that would produce identical chunk **contents** and still share zero **ids** — CDC would measure as a pure regression: more chunks, larger manifests, no dedup. The envelope format is frozen (Part I) so the id cannot stop covering expiry; the file layer is not, so the fix belongs there. Two routes, and the choice is a design decision rather than a detail — **(A)** make chunk envelopes a pure function of content (content-derived `file_id`, fixed `dest`, quantised `expiry`), which changes delivery scoping and store semantics for a partial, time-degrading dedup; or **(B)** have the manifest name **content hashes** rather than envelope ids, with a `hash(body) → envelope id` index in the store, which touches nothing frozen and dedups exactly, at the cost of a real change to the file layer and the authorisation gate. B is the better shape. Either way this is prerequisite work, not part of the chunking change |
...
| M11-P A pending interest that survives the meeting | ✅ | The lease is now scoped to the **object**: `interest_deadline` reads the expiry of the manifest naming the id and clamps it into `[now + INTEREST_LEASE_SECS, now + MAX_INTEREST_LEASE_SECS]`, so an interest lives as long as the chunks could still turn up and no longer. `Node::pending_interests` / `restore_pending_interests` persist it across a restart — waiters are deliberately not carried, since an iface index means nothing on another continent, and the courier needs to remember *what* was wanted, not *who* wanted it. `Node::resume_interests`, driven from `tick` every `INTEREST_RESUME_SECS`, is what makes a carried want get *spoken* somewhere new; there is no other path that would, because a WANT is emitted when a neighbour asks and after a journey that neighbour is gone. Exposed to the browser (`spore_node_pending_interests`, `pendingInterests()`), which is the platform where carrying a want actually matters. Safe to lengthen only because M11-K shipped first: without cancel this would have multiplied exactly the standing pull `fetch-abandoned` measures |
| M11-L A malicious-WANT scenario in `spore-sim` | ✅ | `malicious-want` on a 24-node line, and it found a real one. Recursive pull has three bounds — the parent-manifest gate, interest-table dedupe, and depth — and the first two are checked locally while **depth was whatever the asker wrote**. `DEFAULT_WANT_DEPTH` was only the fallback for a WANT carrying no byte at all, so a stranger writing `255` bought 255 hops instead of 8: measured, an honest depth left **8 of 24** nodes holding an interest and a forged one left **23 of 24**, from a single unsigned frame. M11-P had just made each of those a multi-day obligation re-stated every `INTEREST_RESUME_SECS`, so one frame bought on the order of 46 000 frames of mesh-wide re-asking. Fixed by clamping incoming depth to local policy — `min`, not rejection, since a relayed WANT legitimately carries a decremented depth. The CI threshold is phrased as "no *more* than": the two scenarios are compared against **each other**, so the honest reach may change and the gap may not. Verified to fail with the clamp removed |
