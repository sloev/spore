...
| Solo bring-up smoke test: boot, fresh identity, one self-signed envelope logged over UART, a tick observed firing (E1) | 🟡 partial — **flashed and running** | LOLIN S2 Mini (ESP32-S2FNR2 rev v1.0), 2026-08-25: boots, and the tick loop is firing on schedule (`tick #12` at 55.6 s, a 5 s cadence). **Live heap: 226,396 bytes free** — the runtime number section sizes could never give. Identity and signature lines not yet read: the console *is* the USB port, so boot output is gone before a host can attach. The summary now repeats every 30 s so it can be caught |
