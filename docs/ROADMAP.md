...
| M11-M Content-defined chunking (#256.4) | ✅ | Two premises had to be fixed before the chunking itself meant anything. **(1)** Chunks were not content-addressed: a byte-identical file published twice shared 0 of 16 envelopes, because a chunk's name hashed its whole envelope — including a random per-publish `file_id`, a topic derived from it, and a wall-clock expiry. Split into two namespaces: an **envelope id** names a message and must cover expiry and dest; a **content id** names bytes. **(2)** Boundaries were `mtu - 64` at fixed offsets. MTU-derived meant a Wi-Fi and a LoRa publisher cut the same file differently and shared nothing; fixed offsets meant a one-byte insert shifted every later boundary. `src/cdc.rs` is FastCDC over a derived gear table, with min 1 KiB / avg 4 KiB / max 8 KiB fixed by the protocol — sized against the 16 KiB reassembly floor of a minimal node, not against the largest file. Measured: 400 kB of one byte is ~50 chunks and **two** stored objects; two publishers on a 1400- and a 237-byte link serve each other's fetchers; a one-byte prepend republishes a handful of parts instead of all 44. **Note the dependency it creates** — chunks are now routinely larger than a link's frame, so per-hop splitting is no longer an optimisation but a requirement, and `file-multihop` delivers nothing without it. The browser node has no link fragmentation (see below), so this is now blocking there rather than merely missing |
