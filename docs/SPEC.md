...
- **Sign:** Ed25519 (§2).
- **Seal (baseline, one shot):** libsodium `crypto_box_seal` to the recipient's newest **prekey** (X25519, from ANNOUNCE, signed there). Zero per-message state. Implementations SHOULD rotate prekeys daily and **delete private prekeys after 7 d**, so a seized device cannot read mail older than a week — but see §7.2: this reference implementation does *not*, and a spec that says "SHOULD" while its only implementation says "never" is worth stating out loud rather than leaving as an inference.
- **Sessions: Double Ratchet.** Init: root = KDF(X25519 shared from the first seal). Payload = `[dh_pub:32][n:2][pn:2][ct]`; ct = ChaCha20-Poly1305(mk, nonce=n, ad=header). On a new `dh_pub`: root,ck_in = KDF(root, DH); fresh ratchet key: root,ck_out = KDF(root, DH). Each mk = KDF(ck), then delete it; cache skipped mks ≤ 7 d (out-of-order arrival is normal here). KDF = BLAKE2b. Compromise leaks nothing older than one ratchet turn.
...
- **§7.2 Prekey lifetime — what this implementation actually does.** One prekey per identity, for the life of the identity: `prekey_sec = SHA-256(seed ‖ "spore/prekey/v1")`. There is no "newest", no rotation, no expiry. Deletion would also be meaningless as designed, because the prekey is a pure function of the seed a node persists to restore itself — hold the seed and you re-derive it and open every message ever sealed to that node. So **the one-shot seal has no forward secrecy**; sessions (Double Ratchet, above) do. Getting the §7 property needs prekey secrets stored and deleted independently of the identity seed, which changes what restoring from a seed recovers. Tracked as S-022.
...
## 11. Defaults
hops 16 · expiry 7 d · HELLO 5→80 min Trickle · ANNOUNCE flood ≤ 1/h · prekey rotate 1 d, delete 7 d · path fresh 3 h · seen-set ≥ 30 d · relay airtime ≤ 10% · payload UTF-8. T0 ≈ 60 lines; full T2 ≈ 400 with libsodium.
