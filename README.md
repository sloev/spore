...
Windows, and Android. The core also compiles to `wasm32` for the browser (a
single-threaded event loop driving JS transports — WebRTC, WebSocket,
WebTransport, Web Serial/Bluetooth, Nostr…) and to `esp-idf` for the ESP32.
Adding a medium is a thin `recv`/`send` shim (`bridge::driver::DatagramTransport`);
all the routing, address resolution, and fragmentation stay in the shared lib.
...
| `src/ffi.rs`    | a C ABI over the core primitives (identity, messages, sealing, armor) |
...
| `docs/BRIDGES.md` | every medium SPORE can ride (28-row matrix) and how each maps to the model |
