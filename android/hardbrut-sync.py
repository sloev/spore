...
A handful of mechanical patches (COMPILE_FIXES below) are applied on the way
in, narrowly and loudly, the same way web/hardbrut-import.mjs strips the
...
resolve at all — and `HardbrutButton`/`HardbrutTextField` call
`ProvideTextStyle`/`BasicTextField`/`onFocusChanged` without importing any
of them. Remove a COMPILE_FIXES entry and re-run this script once upstream
fixes the bug it patches; if it no longer applies, this script fails loudly
rather than silently vendoring a broken file.
"""
import re
...
COMPILE_FIXES = [
    ...
    (
        ...
        "HardbrutButton/HardbrutTextField call ProvideTextStyle/BasicTextField/"
        "onFocusChanged without importing any of them — add all three",
    ),
]
