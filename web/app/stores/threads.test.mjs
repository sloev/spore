// ThreadStore rules. No DOM, no client.
//
//   cargo build --release --lib --target wasm32-unknown-unknown
//   node web/app/stores/threads.test.mjs
//
// **These assertions have not changed.** The store moved into Rust (M10-B) and
// this file is the oracle that says the move was behaviour-preserving, so the
// only edit was giving each store a communicator to delegate to. If an
// assertion here had to be relaxed to make the Rust pass, that would be the
// migration failing rather than the test being out of date.
...
function newStore(opts = {}) {
  ...
  return new ThreadStore({ ...opts, comm: () => comm });
}
...
// Real 16-byte envelope ids. The JS store took any string, so these used to be
// placeholders like 'abc123'; the Rust store takes the id the kernel actually
// minted, which is 16 bytes and nothing else. That is a stricter interface, and
// the app already passes real ids — `envelope.id` comes from `sendDirect`.
const ID_A = '0102030405060708090a0b0c0d0e0f10';
const ID_B = 'aabbccddeeff00112233445566778899';
const ID_GHOST = 'ffffffffffffffffffffffffffffffff';
...
await test('a send is a real row carrying the true envelope id', async () => {
  const s = newStore();
  s.send({ id: ID_A, to: ADDR, body: 'hi', at: 5, sealed: true });
  const [m] = s.messages(ADDR);
  assert.strictEqual(m.status, 'queued');
  assert.strictEqual(m.id, ID_A, 'the row carries the id the kernel minted');
  assert.strictEqual(m.self, true);
});

await test('an ack reconciles by id, not by position', async () => {
  const s = newStore();
  s.send({ id: ID_A, to: ADDR, body: 'one', at: 1, sealed: true });
  s.send({ id: ID_B, to: ADDR, body: 'two', at: 2, sealed: true });
  assert.ok(s.setStatus(ID_B, 'acked'));
...
await test('an unknown id reports that it matched nothing', async () => {
  const s = newStore();
  s.send({ id: ID_A, to: ADDR, body: 'x', at: 1, sealed: true });
  assert.strictEqual(s.setStatus(ID_GHOST, 'acked'), false);
});
