...
<style>
  ...
  :root {
    /* HARDBRUT (docs/VISUALDESIGN.md §1) — light-first, same values as
       site/style.css so the docs site and this standalone node read as one
       design language. Resolved hexes rather than a raw-plus-semantic
       indirection: this file ships alone. */
    --bg:#fdfaf2;
    --paper:#ffffff;
    --muted:#666666;
    --yellow:#ffd23f;
    ...
    --display: Impact, "Arial Narrow Bold", Haettenschweiler, system-ui, sans-serif;
    --control-h:48px;
    --control-px:14px;
    --control-py:9px;
    --chip-h:32px;
  ...
  @media (prefers-color-scheme: dark) {
    :root {
      /* Dark mode — inverted ink/paper; --onyellow is the near-black text
         that sits on the unchanged yellow face. */
      --bg:#121210;
      --paper:#1a1a18;
      --muted:#a0a090;
      --yellow:#ffd23f;
  ...
  /* >>> end design tokens <<< */
  /* HARDBRUT compound tokens — hard no-blur shadows. The colour rides --ink, so
     they flip with the theme; the offsets never blur. */
  :root { --shadow: 5px 5px 0 var(--ink); --shadow-sm: 3px 3px 0 var(--ink); --shadow-lg: 7px 7px 0 var(--ink); }
  * { box-sizing: border-box; }
  ...
  h1 { margin:0 0 4px; font-family:var(--display); text-transform:uppercase; letter-spacing:-.02em; line-height:1.1; font-size:26px; display:flex; align-items:center; gap:6px; }
  h1 .s { color:var(--ink); }
  .brand-mark { display:block; flex-shrink:0; }
  .tag { color:var(--muted); margin:0 0 16px; max-width:72ch; }
  .bar { display:flex; gap:8px; flex-wrap:wrap; align-items:center; margin:6px 0 20px; }
  .pill { font-size:12px; color:var(--muted); border:var(--border); border-radius:0; padding:3px 10px; }
  .pill.addr { font-family:var(--mono); color:var(--ink); }
  .card { background:var(--paper); border:var(--border); border-radius:0;
    box-shadow: var(--shadow); padding:24px; margin-bottom:24px; }
  .card h2 { margin:0 0 12px; font-family:var(--display); font-size:14px; font-weight:800; text-transform:uppercase; letter-spacing:.02em; color:var(--ink); }
  .log { font-family:var(--mono); font-size:12.5px; background:var(--bg); border:var(--border);
    border-radius:0; height:220px; overflow-y:auto; padding:12px; white-space:pre-wrap; }
  /* tx/rx are distinguished by side and prefix, and here by weight — never by
     colour alone (HARDBRUT has no success/error hue). */
  .log .rx{color:var(--ink)} .log .tx{color:var(--ink)} .log .relay{color:var(--muted)}
  .log .sys{color:var(--muted)} .log .bad{color:var(--ink);font-weight:800}
  .row { display:flex; gap:12px; margin-top:12px; flex-wrap:wrap; align-items:center; }
  input[type=text], select, textarea { background:var(--paper); color:var(--ink);
    border:var(--border); border-radius:0; box-shadow:var(--shadow-sm); padding:10px 12px; font:inherit; }
  ...
  /* Two button kinds: default (yellow) and cancel (white). Same size, same
     border, same hard shadow — only background differs. Disabled is frozen, not
     recoloured. */
  button{ background:var(--yellow); color:var(--ink); border:var(--border); font-family:var(--display);
    font-weight:800; text-transform:uppercase; letter-spacing:.02em; border-radius:0;
    box-shadow:var(--shadow); padding:10px 24px; cursor:pointer; font:inherit; font-family:var(--display); }
  button:disabled{ opacity:0.45; cursor:not-allowed; box-shadow:none; }
  button.ghost, button.x{ background:var(--paper); color:var(--ink); border:var(--border); box-shadow:var(--shadow-sm); }
  button.ghost:disabled{ opacity:0.45; cursor:not-allowed; box-shadow:none; }
  button.x{ padding:2px 10px; box-shadow:var(--shadow-sm); }
  ...
  code{ font-family:var(--mono); font-size:.9em; background:var(--paper); border:2px solid var(--ink); border-radius:0; padding:1px 5px; }
  .bridge{ border:var(--border); border-radius:0; padding:12px 16px; margin-top:12px; background:var(--paper); }
  .bridge .hd{ display:flex; align-items:center; gap:12px; }
  .bridge .ttl{ font-weight:800; font-size:13.5px; flex:1; }
  .badge{ font-size:10px; font-family:var(--mono); border:2px solid var(--ink); border-radius:0; box-shadow:var(--shadow-sm);
    padding:2px 10px; color:var(--ink); background:var(--yellow); text-transform:uppercase; letter-spacing:.05em; white-space:nowrap; }
  .badge.open{ background:var(--paper); color:var(--ink); }
  .badge.err{ background:var(--ink); color:var(--paper); }
